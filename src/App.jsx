...
const PLANS = [
  {
    ...
    features: ["1 utilisateur", "Factures illimitées", "PDF automatique", "Trésorerie", "Support email"],
    color: "#5BA4FF",
  ...
  },
  {
    ...
    features: ["5 utilisateurs", "Agent IA inclus", "PDF professionnel", "SYSCOHADA complet", "Relances auto", "Support prioritaire"],
    color: "#00E5B0",
  ...
  },
  {
    ...
    features: ["Utilisateurs illimités", "Multi-entreprises", "IA décisionnelle", "OCR documents", "API access", "Support dédié"],
    color: "#FFD060",
    cta: "Choisir Pro",
    trial: false,
  },
...
const db = {
  ...
  async update(table, id, data, token) {
    ...
    if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.message || `Erreur update ${table}`); }
    return res.json();
  },
...
.badge{display:inline-block;padding:3px 10px;border-radius:20px;font-size:10px;font-weight:700;background:${facture.statut === "payée" ? "#e8f9f2" : "#fff8e1"};color:${facture.statut === "payée" ? "#00C48C" : "#F5A623"};border:1px solid ${facture.statut === "payée" ? "#00C48C40" : "#F5A62340"}}
</style></head><body>
...
function LandingPage({ onGetStarted }) {
  ...
  const features = [
    { icon: "◧", title: "Facturation PDF", desc: "Créez des factures professionnelles avec TVA 18% en un clic. PDF téléchargeable instantanément." },
    { icon: "◈", title: "Agent IA autonome", desc: "Dites à l'IA ce que vous voulez faire. Elle exécute. Créer une facture, analyser la trésorerie, relancer un client." },
    { icon: "⊞", title: "SYSCOHADA natif", desc: "Écritures comptables 411/706/443 générées automatiquement. Conforme UEMOA/OHADA." },
    { icon: "◉", title: "CRM intégré", desc: "Gérez clients et prospects depuis une interface claire. Historique complet des transactions." },
    { icon: "◈", title: "Trésorerie en temps réel", desc: "Suivez vos flux entrants et sortants. Solde disponible mis à jour instantanément." },
    { icon: "🔒", title: "Sécurité entreprise", desc: "Données isolées par compte (RLS), authentification sécurisée, audit logs complets." },
  ];

  const testimonials = [
    { name: "NKSIM GROUP", role: "Menuiserie & Aluminium, Lomé", text: "NeoERP a transformé notre gestion. Les factures PDF sont professionnelles et l'IA nous fait gagner des heures chaque semaine.", initials: "NK" },
    { name: "SODECO SA", role: "Commerce général, Lomé", text: "Enfin un ERP pensé pour l'Afrique. SYSCOHADA intégré, Mobile Money, et une interface que mon équipe a maîtrisée en 2 jours.", initials: "SD" },
  ];

  return (
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif", background: T.bg, color: T.text, minHeight: "100vh", overflowY: "auto" }}>
      <style>{`* { box-sizing: border-box; margin: 0; padding: 0; } ::-webkit-scrollbar { width: 3px; } ::-webkit-scrollbar-thumb { background: ${T.border}; }`}</style>
      ...
      <div style={{ padding: "60px 20px 40px", textAlign: "center", maxWidth: 600, margin: "0 auto" }}>
        ...
        <h1 style={{ fontSize: 32, fontWeight: 900, lineHeight: 1.2, marginBottom: 16, letterSpacing: "-0.03em" }}>
          Dirigez votre entreprise<br />
          <span style={{ color: T.accent }}>avec l'intelligence artificielle</span>
        </h1>
        <p style={{ color: T.sub, fontSize: 14, lineHeight: 1.7, marginBottom: 28 }}>
          Le premier ERP autonome pour PME africaines. Facturez, gérez votre comptabilité SYSCOHADA et pilotez votre trésorerie en langage naturel.
        </p>
        <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
          <button onClick={() => onGetStarted("register")} style={{ background: T.accent, border: "none", borderRadius: 10, padding: "13px 24px", color: T.bg, fontWeight: 800, fontSize: 14, cursor: "pointer" }}>
            Démarrer gratuitement →
          </button>
          <button onClick={() => onGetStarted("login")} style={{ background: T.s2, border: `1px solid ${T.border}`, borderRadius: 10, padding: "13px 24px", color: T.text, fontWeight: 600, fontSize: 14, cursor: "pointer" }}>
            Se connecter
          </button>
        </div>
      ...
      <div style={{ padding: "0 20px 40px", maxWidth: 560, margin: "0 auto" }}>
        <div style={{ background: T.s2, border: `1px solid ${T.accentBorder}`, borderRadius: 14, padding: 16 }}>
          <div style={{ color: T.sub, fontSize: 10, fontWeight: 700, letterSpacing: "0.15em", marginBottom: 10 }}>◈ AGENT IA EN ACTION</div>
          {[
            { role: "user", text: "Crée une facture de 450 000 FCFA pour SODECO SA" },
            { role: "ai", text: "✓ Facture FAC-2026-001 créée\n• Client : SODECO SA\n• HT : 450 000 FCFA · TVA : 81 000 FCFA\n• TTC : 531 000 FCFA\n✓ Écritures SYSCOHADA 411/706/443 générées\n✓ PDF disponible au téléchargement" },
          ].map((m, i) => (
            <div key={i} style={{ display: "flex", gap: 8, flexDirection: m.role === "user" ? "row-reverse" : "row", marginBottom: 10 }}>
              {m.role === "ai" && <div style={{ width: 24, height: 24, borderRadius: 6, background: `linear-gradient(135deg, ${T.accent}, #009970)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, flexShrink: 0 }}>◈</div>}
              <div style={{ background: m.role === "user" ? T.s3 : T.bg, border: `1px solid ${m.role === "ai" ? T.accentBorder : T.border}`, borderRadius: m.role === "user" ? "10px 10px 3px 10px" : "10px 10px 10px 3px", padding: "8px 11px", maxWidth: "85%", fontSize: 11, color: T.text, lineHeight: 1.6, whiteSpace: "pre-line" }}>
                {m.text}
              </div>
            </div>
          ))}
      ...
      <div style={{ padding: "0 20px 50px", maxWidth: 600, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <h2 style={{ fontSize: 22, fontWeight: 900, letterSpacing: "-0.02em" }}>Tout ce dont votre PME a besoin</h2>
          <p style={{ color: T.sub, fontSize: 13, marginTop: 8 }}>Conçu pour les entrepreneurs africains</p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {features.map((f, i) => (
            <div key={i} style={{ background: T.s2, border: `1px solid ${T.border}`, borderRadius: 12, padding: "14px 14px" }}>
              <div style={{ fontSize: 20, marginBottom: 8 }}>{f.icon}</div>
              <div style={{ color: T.text, fontWeight: 700, fontSize: 13, marginBottom: 4 }}>{f.title}</div>
              <div style={{ color: T.sub, fontSize: 11, lineHeight: 1.5 }}>{f.desc}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: "0 20px 50px", maxWidth: 600, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <h2 style={{ fontSize: 22, fontWeight: 900, letterSpacing: "-0.02em" }}>Tarifs simples et transparents</h2>
          <p style={{ color: T.sub, fontSize: 13, marginTop: 8 }}>En FCFA · Sans engagement · Annulez quand vous voulez</p>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {PLANS.map((plan) => (
            <div key={plan.id} style={{ background: plan.popular ? `linear-gradient(135deg, ${T.accentDim}, ${T.s2})` : T.s2, border: `2px solid ${plan.popular ? T.accent : T.border}`, borderRadius: 14, padding: "18px 16px", position: "relative" }}>
              {plan.popular && <div style={{ position: "absolute", top: -10, right: 16, background: T.accent, color: T.bg, borderRadius: 20, padding: "3px 10px", fontSize: 10, fontWeight: 800 }}>LE PLUS POPULAIRE</div>}
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                <div>
                  <div style={{ color: plan.color, fontWeight: 800, fontSize: 16 }}>{plan.name}</div>
                  <div style={{ color: T.sub, fontSize: 11, marginTop: 2 }}>{plan.desc}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ color: T.text, fontWeight: 900, fontSize: 20 }}>{plan.price.toLocaleString("fr-FR")} <span style={{ fontSize: 11, color: T.sub, fontWeight: 400 }}>FCFA/mois</span></div>
                  <div style={{ color: T.dim, fontSize: 10 }}>≈ {plan.priceUSD}$/mois</div>
                </div>
              </div>
              <div style={{ marginBottom: 14 }}>
                {plan.features.map((f, i) => (
                  <div key={i} style={{ display: "flex", gap: 7, padding: "4px 0", fontSize: 12, color: T.sub }}>
                    <span style={{ color: plan.color }}>✓</span> {f}
                  </div>
                ))}
              </div>
              <button onClick={() => onGetStarted("register")} style={{ width: "100%", background: plan.popular ? T.accent : T.s3, border: `1px solid ${plan.popular ? T.accent : T.border}`, borderRadius: 9, padding: "11px", color: plan.popular ? T.bg : T.text, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                {plan.cta} {plan.trial && "— 14 jours gratuits"}
              </button>
            </div>
          ))}
        </div>
      ...
      {/* TESTIMONIALS */}
      <div style={{ padding: "0 20px 50px", maxWidth: 600, margin: "0 auto" }}>
        <h2 style={{ fontSize: 20, fontWeight: 900, textAlign: "center", marginBottom: 20 }}>Ce qu'en disent nos clients</h2>
        {testimonials.map((t, i) => (
          <div key={i} style={{ background: T.s2, border: `1px solid ${T.border}`, borderRadius: 12, padding: 16, marginBottom: 12 }}>
            <div style={{ color: T.text, fontSize: 13, lineHeight: 1.6, marginBottom: 12, fontStyle: "italic" }}>"{t.text}"</div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 32, height: 32, borderRadius: 8, background: `linear-gradient(135deg, ${T.blue}, #2060CC)`, display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 800, fontSize: 12 }}>{t.initials}</div>
              <div>
                <div style={{ color: T.text, fontWeight: 600, fontSize: 12 }}>{t.name}</div>
                <div style={{ color: T.sub, fontSize: 10 }}>{t.role}</div>
              </div>
            </div>
          </div>
        ))}
      ...
      {/* CTA FINAL */}
      <div style={{ padding: "30px 20px 60px", maxWidth: 500, margin: "0 auto", textAlign: "center" }}>
        <div style={{ background: `linear-gradient(135deg, ${T.accentDim}, ${T.s2})`, border: `1px solid ${T.accentBorder}`, borderRadius: 16, padding: "28px 20px" }}>
          <h2 style={{ fontSize: 20, fontWeight: 900, marginBottom: 10 }}>Prêt à piloter votre entreprise avec l'IA ?</h2>
          <p style={{ color: T.sub, fontSize: 13, marginBottom: 20 }}>Rejoignez les PME africaines qui utilisent NeoERP AI</p>
          <button onClick={() => onGetStarted("register")} style={{ background: T.accent, border: "none", borderRadius: 10, padding: "13px 28px", color: T.bg, fontWeight: 800, fontSize: 14, cursor: "pointer" }}>
            Démarrer gratuitement →
          </button>
          <div style={{ color: T.dim, fontSize: 11, marginTop: 12 }}>✓ 14 jours gratuits · ✓ Sans carte bancaire</div>
        </div>
      </div>

      {/* FOOTER */}
      <div style={{ borderTop: `1px solid ${T.border}`, padding: "16px 20px", textAlign: "center" }}>
        <div style={{ color: T.dim, fontSize: 11 }}>© 2026 NeoERP AI — NeoTech Solutions — Lomé, Togo</div>
        <div style={{ color: T.dim, fontSize: 10, marginTop: 4 }}>ERP intelligent pour PME africaines · SYSCOHADA · UEMOA</div>
      </div>
    </div>
  );
}
    ...
    <div style={{ height: "100vh", background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ width: "min(400px, 100%)" }}>
        ...
        <div style={{ background: T.s2, border: `1px solid ${T.border}`, borderRadius: 14, padding: 20 }}>
          <div style={{ display: "flex", marginBottom: 16, background: T.s3, borderRadius: 8, padding: 3 }}>
            {["login", "register"].map(m => <button key={m} onClick={() => { setMode(m); setError(""); }} style={{ flex: 1, background: mode === m ? T.accent : "none", border: "none", borderRadius: 6, padding: "7px", color: mode === m ? T.bg : T.sub, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>{m === "login" ? "Se connecter" : "Créer un compte"}</button>)}
          </div>
...
async function runAI(msg, history, onStatus, token, userId) {
  ...
  const res = await fetch("/api/ai", {
    ...
    body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 800, system: `Tu es NeoERP AI Agent — ERP autonome SYSCOHADA/OHADA PME africaines. TOOLS sur une ligne TOOL:{json}: creer_client, creer_facture, lister_factures, lister_clients, analyser_tresorerie, enregistrer_operation, statistiques. Réponds en français. Montants FCFA.`, messages: [...history, { role: "user", content: msg }] }),
  });
    ...
    <div style={{ padding: 18, height: "100%", overflowY: "auto" }}>
      ...
      {showForm && (
        <div style={{ background: T.s2, border: `1px solid ${T.accentBorder}`, borderRadius: 11, padding: 13, marginBottom: 13 }}>
          <div style={{ background: T.accentDim, borderRadius: 7, padding: "5px 10px", marginBottom: 8, fontSize: 10, color: T.accent }}>✓ TVA 18% auto · SYSCOHADA 411/706/443 · PDF</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
            <input value={form.client_nom} onChange={e => setForm(p => ({ ...p, client_nom: e.target.value }))} placeholder="Nom du client *" style={{ background: T.s3, border: `1px solid ${T.border}`, borderRadius: 7, padding: "8px 11px", color: T.text, fontSize: 12, outline: "none" }} />
            <input value={form.montant_ht} onChange={e => setForm(p => ({ ...p, montant_ht: e.target.value }))} placeholder="Montant HT en FCFA *" type="number" style={{ background: T.s3, border: `1px solid ${T.border}`, borderRadius: 7, padding: "8px 11px", color: T.text, fontSize: 12, outline: "none" }} />
            {form.montant_ht > 0 && <div style={{ background: T.accentDim, borderRadius: 7, padding: "5px 10px", fontSize: 11, color: T.accent }}>TTC : {Math.round(Number(form.montant_ht) * 1.18).toLocaleString("fr-FR")} FCFA</div>}
            <input value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Description" style={{ background: T.s3, border: `1px solid ${T.border}`, borderRadius: 7, padding: "8px 11px", color: T.text, fontSize: 12, outline: "none" }} />
            <input value={form.date_echeance} onChange={e => setForm(p => ({ ...p, date_echeance: e.target.value }))} type="date" style={{ background: T.s3, border: `1px solid ${T.border}`, borderRadius: 7, padding: "8px 11px", color: T.text, fontSize: 12, outline: "none" }} />
          </div>
      ...
      {loading ? <div style={{ textAlign: "center", padding: 40, color: T.sub, fontSize: 12 }}>Chargement…</div>
        : factures.length === 0 ? <div style={{ textAlign: "center", padding: 40, color: T.sub, fontSize: 13 }}>Aucune facture.</div>
            ...
            {factures.map((f, i) => (
              <div key={f.id} style={{ padding: "10px 13px", borderBottom: i < factures.length - 1 ? `1px solid ${T.border}` : "none" }}>
                ...
                <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                  {f.statut !== "payée" && <button onClick={() => markPaid(f.id)} style={{ background: T.accentDim, border: `1px solid ${T.accentBorder}`, borderRadius: 5, padding: "3px 9px", color: T.accent, fontSize: 10, cursor: "pointer", fontWeight: 600 }}>✓ Payée</button>}
                  <button onClick={() => generatePDF(f)} style={{ background: T.blueDim, border: `1px solid ${T.blue}30`, borderRadius: 5, padding: "3px 9px", color: T.blue, fontSize: 10, cursor: "pointer", fontWeight: 600 }}>⬇ PDF</button>
                </div>
              </div>
            ))}
          </div>}
...
function Dashboard({ token, onNavigate, refreshKey, plan }) {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        const ent = Array.isArray(ops) ? ops.filter(o => o.type === "entrée").reduce((s, o) => s + (o.montant || 0), 0) : 0;
        const sor = Array.isArray(ops) ? ops.filter(o => o.type === "sortie").reduce((s, o) => s + (o.montant || 0), 0) : 0;
        setStats({ clients: Array.isArray(clients) ? clients.length : 0, factures: Array.isArray(factures) ? factures.length : 0, impaye: imp, solde: ent - sor });
      } catch (e) {}
    };
    run();
  }, [refreshKey]);
  ...
  return (
    <div style={{ padding: 18, height: "100%", overflowY: "auto" }}>
      <div style={{ background: T.s2, border: `1px solid ${currentPlan.color}30`, borderRadius: 10, padding: "8px 12px", marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}>
        <div style={{ color: currentPlan.color, fontSize: 10, fontWeight: 700 }}>Plan {currentPlan.name}</div>
        <div style={{ color: T.dim, fontSize: 10 }}>·</div>
        <div style={{ color: T.sub, fontSize: 10 }}>{currentPlan.price.toLocaleString("fr-FR")} FCFA/mois</div>
      </div>
      <button onClick={() => onNavigate("agent")} style={{ width: "100%", background: `linear-gradient(135deg, #0A1A14, #0D2018)`, border: `1px solid ${T.accent}30`, borderRadius: 11, padding: "13px 16px", display: "flex", alignItems: "center", gap: 13, cursor: "pointer", marginBottom: 16, textAlign: "left" }}>
        ...
        <div><div style={{ color: T.accent, fontWeight: 800, fontSize: 13 }}>Agent IA — Actions réelles</div><div style={{ color: T.sub, fontSize: 11 }}>SYSCOHADA · PDF · Sécurisé</div></div>
      ...
      {!stats ? <div style={{ textAlign: "center", padding: 40, color: T.sub, fontSize: 12 }}>Chargement…</div> : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 11 }}>
          {[{ label: "Clients", val: stats.clients, color: T.blue, view: "clients" }, { label: "Factures", val: stats.factures, color: T.accent, view: "factures" }, { label: "Impayés", val: fmtShort(stats.impaye) + " F", color: T.gold, view: "factures" }, { label: "Trésorerie", val: fmtShort(stats.solde) + " F", color: stats.solde >= 0 ? T.accent : T.red, view: "tresorerie" }].map((k, i) => (
...
const NAV = [{ id: "dashboard", icon: "⬡", label: "Tableau de bord" }, { id: "agent", icon: "◈", label: "Agent IA" }, { id: "clients", icon: "◉", label: "Clients" }, { id: "factures", icon: "◧", label: "Facturation" }, { id: "tresorerie", icon: "◈", label: "Trésorerie" }];

export default function NeoERPV8() {
  ...
  const handleAuth = (token, refresh_token, user, company, plan) => {
    const s = { token, refresh_token, user, company, plan: plan || "starter", expires_at: Date.now() + 3600000 };
    setSession(s); setScreen("app");
  };

  const handleLogout = async () => {
    try { await auth.signOut(session.token); } catch (e) {}
    auth.clearSession(); setSession(null); setScreen("landing");
  };
  ...
  if (sessionLoading) return (
    <div style={{ height: "100vh", background: T.bg, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ color: T.sub, fontSize: 12 }}>Chargement…</div>
      </div>
    </div>
  );
  ...
  const renderView = () => {
    switch (view) {
      case "dashboard": return <Dashboard {...props} onNavigate={setView} plan={session.plan} />;
      ...
      default: return <Dashboard {...props} onNavigate={setView} plan={session.plan} />;
    }
  };
    ...
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif", background: T.bg, color: T.text, height: "100vh", display: "flex", overflow: "hidden" }}>
      ...
      <div style={{ width: collapsed ? 50 : 200, flexShrink: 0, background: T.s1, borderRight: `1px solid ${T.border}`, display: "flex", flexDirection: "column", transition: "width 0.22s", overflow: "hidden" }}>
        <div style={{ padding: "13px 11px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {!collapsed && (<><div><div style={{ color: T.text, fontWeight: 800, fontSize: 12 }}>NeoERP AI</div><div style={{ color: T.accent, fontSize: 9, fontWeight: 600 }}>● V8 · SaaS · PDF</div></div><button onClick={() => setCollapsed(true)} style={{ marginLeft: "auto", background: "none", border: "none", color: T.dim, cursor: "pointer" }}>⇤</button></>)}
        ...
        <nav style={{ flex: 1, padding: "7px 5px" }}>
          {NAV.map(item => <button key={item.id} onClick={() => setView(item.id)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: collapsed ? "9px 0" : "9px 9px", justifyContent: collapsed ? "center" : "flex-start", background: view === item.id ? T.accentDim : "none", border: view === item.id ? `1px solid ${T.accentBorder}` : "1px solid transparent", borderRadius: 7, cursor: "pointer", color: view === item.id ? T.accent : T.sub, fontSize: 12, marginBottom: 1, whiteSpace: "nowrap", overflow: "hidden" }}><span style={{ fontSize: 13, flexShrink: 0 }}>{item.icon}</span>{!collapsed && <span>{item.label}</span>}</button>)}
        </nav>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: "0 16px", height: 43, borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", background: T.s1, flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
            ...
            <span style={{ color: T.sub, fontSize: 10 }}>🔒 V8 · SaaS</span>
          </div>
        </div>
